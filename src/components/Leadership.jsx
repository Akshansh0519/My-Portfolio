...
const LeadershipItem = ({ item, index }) => {
  ...
  return (
    <div className="relative flex flex-col md:flex-row items-center justify-between mb-12 md:mb-16 w-full group">
      {/* Timeline line dot */}
      <div className="absolute left-4 md:left-1/2 -translate-x-1/2 w-4 h-4 bg-cyan-400 rounded-full border-4 border-black z-30 shadow-[0_0_15px_#22d3ee] group-hover:scale-125 transition-transform duration-300" />
      ...
      >
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 hover:border-cyan-500/30 hover:shadow-[0_15px_35px_rgba(0,240,255,0.08)] transition-all duration-500">
          <div className={`flex flex-wrap gap-2 items-center mb-3 ${isEven ? 'md:justify-end' : 'md:justify-start'}`}>
            <span className="bg-cyan-500/10 text-cyan-400 text-[10px] font-black tracking-widest uppercase py-1 px-3 rounded-full border border-cyan-500/20">
              {item.badge}
            </span>
          </div>
          
          <h3 className="text-white text-xl font-black mb-1 tracking-tight group-hover:text-cyan-400 transition-colors">
            {item.title}
          </h3>
          <p className="text-emerald-400 text-xs font-bold font-mono tracking-wider uppercase mb-4">
            {item.role}
          </p>
    ...
    <section className="bg-[#0a0a0a] pt-24 pb-32 px-6 md:px-12 w-full relative overflow-hidden font-sans bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:80px_80px]">
      ...
      <div className="max-w-6xl mx-auto relative z-20">
        ...
        <div className="relative w-full">
          ...
          <div className="absolute left-4 md:left-1/2 -translate-x-1/2 top-2 bottom-2 w-[2px] bg-gradient-to-b from-cyan-400 via-emerald-500/50 to-white/10" />
