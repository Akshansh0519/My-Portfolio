<p align="center">
  ...
  <em>Real-Time Collaboration & Document RAG Engine</em>
</p>
...
SyncNexus is a **production-grade collaborative workspace platform** featuring real-time messaging, shared document libraries, and a **Retrieval-Augmented Generation (RAG)** AI assistant. 

Unlike typical tutorial chat applications, SyncNexus implements a complete, scalable backend architecture — from direct-to-S3 uploads and keyset pagination to distributed real-time messaging via Redis Pub/Sub and resilient background workers with exponential backoff.

The system integrates **ChromaDB** for vector storage, **BullMQ** for job processing, and **Gemini** for embedding and LLM generation, all evaluated through a robust suite of integration tests covering concurrency, cross-instance state, and failure recovery.
